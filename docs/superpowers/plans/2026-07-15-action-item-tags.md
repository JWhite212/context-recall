...
- **Migration is v22.** The base branch is `main` at `SCHEMA_VERSION = 21` (PR #71 merged). Bump to 22; un-head the v21 block (its `PRAGMA user_version = {SCHEMA_VERSION}` becomes literal `21`) and add a new `if current_version < 22:` block. (SQLite `ALTER TABLE ADD COLUMN` cannot add a FK constraint to an existing table — the FK is declarative-only via `_safe_add_column`; that's acceptable and matches how `meetings.client_id`/`project_id` were added. Because no real `ON DELETE SET NULL` exists, `delete_client`/`delete_project` null `action_items.client_id`/`project_id` by hand, exactly as they already do for `meetings`.)
